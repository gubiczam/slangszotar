// frontend/src/pages/SlangDetailByTitle.js
...
function SlangDetailByTitle() {
  ...
  useEffect(() => {
    const fetchSlang = async () => {
      try {
        const response = await axios.get(`/slangs/title/${encodeURIComponent(title)}`);
  ...
  useEffect(() => {
    ...
    const fetchRelatedSlangs = async () => {
      try {
        // Kapcsolódó szlengek keresése közös címkék alapján
...
export default SlangDetailByTitle;
